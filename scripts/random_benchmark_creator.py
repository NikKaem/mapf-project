...
def output(instance, name, nodes, robots):
	...
	benchmark_specs = ['benchmark-' + str(amount+1), nodes, robots]
	with open(PATH + '/specs_benchmarks.csv', 'a') as file:
		writer = csv.writer(file)
		writer.writerow(benchmark_specs)

size_x = int(sys.argv[1])
size_y = int(sys.argv[2])
n_rob = int(sys.argv[3])

map, robots, shelves, name = gen_benchmark_data(size_x,size_y,n_rob)
instance = format_output(map, robots, shelves)

output(instance, name, map[0]*map[1], len(robots))
